...
**The JPEG quality finding** is a good example of what joint human-AI iteration produces: AI adviced against higher JPEG quality even when it suggested that it should help signal fidelity but reasoned that TCP congestion from high bitrate would hurt frame timing. This could not be resolved until empirical data from two back-to-back test sessions — one at Q=0.5, one at Q=0.85 — settled it. PNG without compression was also tried but trashed due to lower camera fps. The override was human judgement and the validation was the terminal.
